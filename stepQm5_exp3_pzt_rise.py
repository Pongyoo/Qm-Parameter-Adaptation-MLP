# ============================================================
# stepQm5_exp3_pzt_rise.py
# Step 5: Train MLP to predict Qm_slow from
#         baseline features (current + PZT) + PZT_rise_from_start
#
# Purpose:
#   - Keep the strong baseline: current + PZT
#   - Add only one extra thermal feature: PZT_rise_from_start
#   - Compare fairly against baseline PZT-only version
#   - Keep train/test split strictly time-based (70/30)
#   - Fit scaler on train only
#   - Save predictions for ALL samples so Step 6 can evaluate later
#
# Input:
#   processed_Qm/stepQm4d_merge_temp/data/<BASE>_qm_required_with_temp.csv
#
# Output:
#   processed_Qm/stepQm5_pzt_rise/data/<BASE>_stepQm5_pred.csv
#   processed_Qm/stepQm5_pzt_rise/data/<BASE>_stepQm5_metrics.json
#   processed_Qm/stepQm5_pzt_rise/plots/*.png
# ============================================================
...
IN_CSV = os.path.join(
    SESSION_ROOT, "stepQm4d_merge_temp", "data",
    f"{BASE}_qm_required_with_temp.csv"
)

OUT_DIR  = os.path.join(SESSION_ROOT, "stepQm5_pzt_rise")
...
def rolling_mean_std_causal(x, win):
    ...
    m = m.bfill().ffill().to_numpy(dtype=np.float64)
    v = v.bfill().ffill().to_numpy(dtype=np.float64)
    return m, v
...
def main():
    ...
    required_cols = ["I_rms", "Qm_slow", "PZT"]
    ...
    print("  features  = baseline + PZT_rise_from_start")
    ...
    out = pd.DataFrame({
        ...
        "PZT": pzt.astype(np.float32),
        "PZT_rise_from_start": pzt_rise.astype(np.float32),

        "is_train": np.isin(np.arange(N), idx_tr).astype(np.int32),
        "is_test": np.isin(np.arange(N), idx_te).astype(np.int32),
    })

    for c in ["A_meas", "A_phys", "A_hat", "y_meas", "y_phys", "y_hat", "r_before", "r_after", "Room_temp", "Tool_temp"]:
        if c in df.columns:
            out[c] = df[c].to_numpy()
    ...
    metrics = {
        ...
        "note": "Baseline + one extra thermal feature: PZT_rise_from_start"
    }
    ...
    plt.xlabel("epoch")
    plt.ylabel("loss")
    plt.title(f"{BASE} | training curve (baseline + PZT_rise)")
    ...
    plt.xlabel("Time (s)")
    plt.ylabel("Qm (arb.)")
    plt.title(f"{BASE} | TRAIN ONLY | Qm_slow vs Qm_pred (baseline + PZT_rise)")
    plt.grid(True, alpha=0.25)
    plt.legend()
    plt.tight_layout()
    ...
    # 3) extra debug plot: Qm_pred vs PZT_rise_from_start
    plt.figure(figsize=(6, 5))
    plt.scatter(pzt_rise[idx_tr], pred_tr, s=8, alpha=0.45, label="train")
    if len(idx_te) > 0:
        plt.scatter(pzt_rise[idx_te], pred_te, s=8, alpha=0.35, label="test")
    plt.xlabel("PZT_rise_from_start")
    plt.ylabel("Qm_pred")
    plt.title(f"{BASE} | Qm_pred vs PZT_rise_from_start")
    plt.grid(True, alpha=0.25)
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(PLOT_DIR, f"{BASE}_QmPred_vs_PZTrise.png"), dpi=160)
    plt.close()

    if has_A:
        # 4) amplitude overlay - TRAIN only
        plt.figure(figsize=(12, 4))
        ...
        plt.title(f"{BASE} | TRAIN ONLY | Amplitude overlay (baseline + PZT_rise)")
        plt.grid(True, alpha=0.25)
        plt.legend()
        plt.tight_layout()
        ...
        plt.title(f"{BASE} | TRAIN ONLY | y-space overlay (baseline + PZT_rise)")
        plt.grid(True, alpha=0.25)
        plt.legend()
        plt.tight_layout()
        ...
        # 6) residual before/after - TRAIN only
        plt.figure(figsize=(12, 4))
        ...
        plt.xlabel("Time (s)")
        plt.ylabel("r_y")
        plt.title(f"{BASE} | TRAIN ONLY | y-residual before vs after (baseline + PZT_rise)")
        plt.grid(True, alpha=0.25)
        plt.legend()
        plt.tight_layout()
    ...
    print("\n================ StepQm5 DONE (baseline + PZT_rise_from_start) ================")
    ...
    print("\nNote:")
    print("  - baseline current + PZT kept")
    print("  - added only one new thermal feature: PZT_rise_from_start")
    print("  - final test-only claim must still come from Step 6")
    print("===============================================================================")
