# ============================================================
# stepQm4d_exp3_merge_temp_for_step5.py
...
STEP4C_CSV = os.path.join(
    SESSION_ROOT, "stepQm4c_boundcheck", "data",
    f"{BASE}_qm_required_boundcheck.csv"
)

# Temp source for exp3
# แนะนำใช้ full.csv เพราะมี PZT + Room_temp + Tool_temp ครบ
TEMP_CSV = r"C:\Users\ploy\Desktop\raw_data\exp1(2025.10.28)\3nl-fix17.37.csv"

OUT_DIR = os.path.join(SESSION_ROOT, "stepQm4d_merge_temp")
...
def build_temp_time_axis(df_temp):
    # case 1: numeric time already in seconds
    tcol_num = pick_col(df_temp, ["t_sec", "Time", "time", "t"])
    if tcol_num is not None:
        s_num = pd.to_numeric(df_temp[tcol_num], errors="coerce")
        if s_num.notna().mean() > 0.95:
            return s_num.to_numpy(dtype=float), tcol_num

    # case 2: clock-like string time, e.g. 22:20.7 or 03:04.4
    tcol_clock = pick_col(df_temp, ["Time", "time", "clock_time"])
    if tcol_clock is not None:
        t = parse_clock_time_to_seconds(df_temp[tcol_clock])
        return t, tcol_clock

    raise KeyError("Cannot build time axis from TEMP_CSV.")
...
def main():
    ...
    print("temp time col used =", tt_col)
    print("temp time min/max =", float(np.nanmin(tt)), float(np.nanmax(tt)))
