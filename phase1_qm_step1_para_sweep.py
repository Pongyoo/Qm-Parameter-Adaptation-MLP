# ============================================================
# stepQm1p_exp3_build_params_from_sweep_fft_csv.py
...
SWEEP_CSV_PATH = r"E:\raw_data\exp5(2026.3.27)\1.5_swp30s_exp5.csv"
SESSION_TAG = "1.5_swp30s_exp5"
SESSION_ROOT = os.path.join(ROOT, "1.5_fix18.0_processed_Qm", "stepQm1_para_sweep", SESSION_TAG)

OUT_DIR = os.path.join(SESSION_ROOT, "stepQm1_params_sweep")
os.makedirs(OUT_DIR, exist_ok=True)

LEVEL_NAME = "1.5_swp30s_exp5"
...
print("\n================ StepQm1p EXP3: Sweep Params (CSV) =================")
print("SWEEP_CSV_PATH =", SWEEP_CSV_PATH)
print("EXISTS?        =", os.path.isfile(SWEEP_CSV_PATH))

if not os.path.isfile(SWEEP_CSV_PATH):
    raise FileNotFoundError(SWEEP_CSV_PATH)
...
required_cols = ["Current", "Displacement"]
missing_cols = [c for c in required_cols if c not in df.columns]
if missing_cols:
    raise KeyError(f"Missing required columns in CSV: {missing_cols}")

cur = pd.to_numeric(df["Current"], errors="coerce").to_numpy(dtype=np.float64)
disp = pd.to_numeric(df["Displacement"], errors="coerce").to_numpy(dtype=np.float64)
...
if len(cur) < NSEG or len(disp) < NSEG:
    raise RuntimeError(
        f"Not enough valid samples for Welch. Need at least NSEG={NSEG}, got {len(cur)}"
    )
